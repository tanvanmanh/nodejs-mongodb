...
async function main() {
    ...
    try {
        ...
        const addedItemQuery = await circulationRepo.getById(addedItem.insertedId.toString());
        assert.deepEqual(addedItemQuery, newItem);

        const updatedItem = await circulationRepo.update(addedItem.insertedId.toString(), {
        ...
        assert.equal(updatedItem.value.Newspaper, 'My new paper');
